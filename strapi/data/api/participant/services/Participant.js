...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Participant.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Participant.query(buildQuery({ model: Participant, filters }))
  ...
  count: (params) => {
    ...
    return Participant.query(buildQuery({ model: Participant, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Participant.forge(params).save(data);
